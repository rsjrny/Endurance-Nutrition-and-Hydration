...
# reqcalories, reqsodium, reqwater = utils.calc_requirements(inwater=caloriesV,
#                                                            insod=sodiumV,
#                                                            incal=waterV,
#                                                            dectime=dectime)
...
column2 = [[sg.Text('Run Pace')],
           [sg.InputText(size=8, key='-INPACE-', default_text=paceV, tooltip='Enter your desired run pace')],
           [sg.Text('')],
           [sg.Text('   ', size=15)],
           [sg.Text('')],
           [sg.Text('   ', size=15)]
...
column3 = [[sg.Text('Water/hr. (ml)')],
           [sg.InputText(size=13, key='-INWATER-', default_text=waterV,
...
column4 = [[sg.Text('Calories/hr.')],
           [sg.InputText(size=12, key='-INCALORIES-', default_text=caloriesV,
...
column5 = [[sg.Text('Sodium/hr. (mg)')],
           [sg.InputText(size=15, key='-INSODIUM-', default_text=sodiumV,
...
while True:
    ...
    if event == '-NUTRITIONPROD-' and len(values['-NUTRITIONPROD-']):
        # print('in: len(values[-NUTRITIONPROD-]', len(values['-NUTRITIONPROD-'][0]))
    ...
    if event == 'Delete Entry':  # perform quantity Update
        if not len(values['-NUTRITIONPROD-']) or values['-NUTRITIONPROD-'] == '':
    ...
    #     mytheme = sg.popup_get_text('Enter the desired theme')
    #     if mytheme not in theme_list:
    #         sg.popup_error('The theme selected is not valid. Use the Settings...Theme Previewer to display all valid theme names.')
    #     else:
    #         # print('selected theme: ', mytheme)
    #         sg.theme(mytheme)
    #         utils.set_config(configs, 'DEFAULT', 'theme', mytheme)
    #         utils.rewrite_config_file(inifile, configs, values)
    #         window.refresh()
    #     pass
    # if event == 'Display Theme Choices':
    #     theme_name = sg.theme_previewer()
    #     # print(theme_name)
    #     pass
